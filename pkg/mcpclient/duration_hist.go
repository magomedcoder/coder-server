...
func writeCallToolDurationHistogram(w io.Writer) {
	fmt.Fprintf(w, "# HELP coder_mcp_call_tool_duration_seconds Длительность MCP tools/call (сек)\n")
	fmt.Fprintf(w, "# TYPE coder_mcp_call_tool_duration_seconds histogram\n")
	le := []string{"0.05", "0.2", "1", "5"}
	for i := range callToolHistBucket {
		fmt.Fprintf(w, "coder_mcp_call_tool_duration_seconds_bucket{le=%q} %d\n", le[i], callToolHistBucket[i].Load())
	}

	cnt := callToolDurationCount.Load()
	fmt.Fprintf(w, "coder_mcp_call_tool_duration_seconds_bucket{le=\"+Inf\"} %d\n", cnt)
	sumSec := float64(callToolDurationSumNs.Load()) / 1e9
	fmt.Fprintf(w, "coder_mcp_call_tool_duration_seconds_sum %g\n", sumSec)
	fmt.Fprintf(w, "coder_mcp_call_tool_duration_seconds_count %d\n", cnt)
}
