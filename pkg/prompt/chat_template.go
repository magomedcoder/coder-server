...
func RenderMatchedPreset(preset *template.MatchedPreset, messages []*domain.Message) (string, error) {
	if preset == nil {
	...
	if err := tmpl.Execute(&buf, template.Values{Messages: msgs}); err != nil {
		return "", fmt.Errorf("runnerprompt: пресет %q: %w", preset.Name, err)
	}
...
func PresetStopSequences(chatTemplateJinja string) ([]string, error) {
	...
	return template.PresetStopSequences(preset), nil
}

func messagesForTemplate(messages []*domain.Message) ([]template.Message, error) {
...
func messageToTemplate(m *domain.Message) (template.Message, error) {
	tm := template.Message{
		Role:    string(m.Role),
		Content: m.Content,
	}
	...
	case domain.MessageRoleAssistant:
		...
		if tcJSON != "" {
			...
			if err != nil {
				return template.Message{}, fmt.Errorf("разбор tool_calls: %w", err)
			}
