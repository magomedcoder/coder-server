...
func (m *StreamMeta) AsChunk() ChatStreamChunk {
...
func BuildStreamMetaFullDocument(fileID int64, extractedText string) (*StreamMeta, error) {
...
func BuildStreamMetaVector(
	...
	for _, sc := range scored {
		...
		cw := ragChunkWire{
			...
			Score:      sc.Score,
			IsNeighbor: sc.Score <= rag.NeighborOnlyChunkScore/10,
		}
		...
		if ps, ok := rag.IntFromMeta(meta, "pdf_page_start"); ok {
			cw.PdfPageStart = ps
		}

		if pe, ok := rag.IntFromMeta(meta, "pdf_page_end"); ok {
			cw.PdfPageEnd = pe
		}
