...
func runLoop(ctx context.Context, cfg Config, maxRounds int, out chan<- chatstream.ChatStreamChunk) {
	...
	for round := range maxRounds {
		...
		ch, err := cfg.LLM.SendMessageOnRunner(ctx, cfg.RunnerAddr, cfg.SessionID, cfg.Model, history, cfg.StopSequences, cfg.TimeoutSeconds, toolloop.RunnerInferenceParams(gp))
		if err != nil {
			sendErr(err)
			return
		}
